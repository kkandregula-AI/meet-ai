...
1. **Firebase**: create a project → Realtime Database → paste the config into
   ...
   ```
2. **Anthropic key**: in Vercel → Settings → Environment Variables, add
...
3. **Deploy**: `vercel --prod` from this folder. Done.
...
- **Participants strip** under the header shows everyone live — photo if they
  added one, initials otherwise; moderator gets a violet ring.
- **Photo in audio mode**: optional, picked in the lobby (📷 card), centre-
  cropped to 96px JPEG (~5–8KB) and synced via the participant record.
- **🎥 Video (opt-in)**: WebRTC mesh with Firebase as the signaling channel
  (offer/answer/ICE under `rooms/{code}/signals`, consumed on read). Google
  STUN only — fine for most networks; add a TURN server to `ICE` in index.html
  for strict corporate NATs. Mesh topology: keep it to ~4 participants.
  Your photo shows in the tile until the remote stream starts playing.

## Troubleshooting: "we can't see each other"

If a joining device shows the room title **"Local room"**, that device is in
demo mode — `CONFIG.FIREBASE` still has placeholder values, so there is no
realtime sync at all (the app now shows a warning banner for this). Fix:
create a Firebase project → Realtime Database → paste the real config into
`CONFIG.FIREBASE` → redeploy.
